...
- Fix applied: Allègement de `PROJECT_STATUS` (focus état/priorités/risques/journal récent) et création d’une archive dédiée pour l’historique détaillé.
- Status: ✅ Resolved
---
...
---
**[2026-03-04] — Régression JSX pendant la passe d’uniformisation design dashboard**
- File(s) affected: `src/app/(dashboard)/club/reservations/ClubReservationsPanel.tsx`, `src/app/(dashboard)/layout.tsx`, `src/app/(dashboard)/DashboardSidebarNav.tsx`, `src/app/(dashboard)/DashboardMobileNav.tsx`, `src/app/(dashboard)/club/clients/ClubClientsPanel.tsx`
- Error: Build TypeScript cassé (`JSX element 'section' has no corresponding closing tag`) après une passe de remplacement de styles.
- Root cause: Duplication accidentelle d’une balise d’ouverture `<section>` dans le panneau réservations lors du patch d’harmonisation visuelle.
- Fix applied: Suppression de la balise dupliquée + validation diagnostics sur tous les fichiers touchés; uniformisation design conservée (tokens, bordures, états actifs).
- Status: ✅ Resolved
---

---
**[2026-03-04] — Incohérence d’accent visuel dans le module VIP**
- File(s) affected: `src/app/(dashboard)/vip/page.tsx`, `src/app/(dashboard)/vip/invitations/page.tsx`, `src/app/(dashboard)/vip/profile/page.tsx`, `src/app/(dashboard)/vip/safety/page.tsx`, `src/app/(dashboard)/promoter/commissions/PromoterCommissionsPanel.tsx`, `src/app/(dashboard)/promoter/promo/PromoterPromoPanel.tsx`, `src/app/(dashboard)/promoter/guestlist/GuestListClient.tsx`
- Error: Le module VIP conservait des accents gold résiduels (focus/bordures/inputs) au lieu d’un langage visuel rose homogène; promoteur présentait des opacités de surface hétérogènes.
- Root cause: Uniformisation initiale concentrée sur le shell dashboard et le rôle club, sans passe dédiée systématique sur les rôles `vip` et `promoter`.
- Fix applied: Réalignement des accents VIP en rose (focus rings, bordures, checkbox accent, cartes) + normalisation légère des surfaces promoteur (opacités de bordure), puis validation diagnostics sans erreur.
- Status: ✅ Resolved
---

---
**[2026-03-04] — Incohérence visuelle persistante sur les pages publiques (acquisition)**
- File(s) affected: `src/app/page.tsx`, `src/app/(public)/clubs/page.tsx`, `src/app/(public)/clubs/[slug]/page.tsx`, `src/app/(public)/clubs/[slug]/events/[eventId]/page.tsx`
- Error: Les pages publiques utilisaient encore des variations de fond/contours/focus non homogènes avec le design system dark luxury consolidé sur les dashboards.
- Root cause: Uniformisation conduite d’abord par rôles dashboard, laissant une dérive visuelle résiduelle sur le périmètre public.
- Fix applied: Alignement des pages publiques sur les mêmes primitives (fonds #050508, bordures gold low-opacity, focus rings cohérents, sections/cards recalibrées), puis validation diagnostics sur les routes modifiées.
- Status: ✅ Resolved
---

---
**[2026-03-04] — Micro-écarts responsive finaux (mobile safe-area + densité CTA/typo)**
- File(s) affected: `src/app/(dashboard)/DashboardMobileNav.tsx`, `src/app/page.tsx`, `src/app/(public)/clubs/page.tsx`, `src/app/(public)/clubs/[slug]/events/[eventId]/page.tsx`
- Error: Derniers écarts de finition mobile observés (barre basse sans safe-area iOS, CTA hero limite en touch target, densité typographique/padding légèrement inégale sur public mobile).
- Root cause: Les passes précédentes ont priorisé la cohérence colorimétrique et structurelle avant la finition responsive fine.
- Fix applied: Ajout du padding safe-area sur la nav mobile, CTA hero porté à `min-h-12`, ajustement du titre hero mobile et harmonisation des paddings mobiles publics.
- Status: ✅ Resolved
---

---
**[2026-03-04] — Parcours public incomplet entre page club et page événement**
- File(s) affected: `src/app/(public)/clubs/[slug]/page.tsx`
- Error: Les cartes “Programmation en avant” n’ouvraient pas de route événement publique, ce qui cassait le flow attendu `club → event`.
- Root cause: Données de programmation statiques sans identifiant événement exploitable pour construire les URLs publiques.
- Fix applied: Fetch des événements publiés par club (`events`), génération de liens `/clubs/[slug]/events/[eventId]` sur chaque carte, fallback vers `/reserve` en absence d’événement.
- Status: ✅ Resolved
---

---
**[2026-03-04] — Vérification E2E club → event bloquée par visibilité data publique**
- File(s) affected: `src/lib/supabase/serverAdmin.ts`, `src/app/(public)/clubs/[slug]/page.tsx`, `src/app/(public)/clubs/[slug]/events/[eventId]/page.tsx`
- Error: Les pages publiques restaient sur fallback malgré des événements seedés publiés, rendant la validation HTTP club → event non déterministe.
- Root cause: Incohérence de visibilité runtime des données publiques (lookup événements non fiable selon environnement), combinée à l’absence de fallback route explicite vers une page event.
- Fix applied: Ajout d’un client Supabase admin server-side dédié pour les lectures publiques, normalisation typée de la relation `event_tables -> tables`, forçage du rendu dynamique des routes publiques, et fallback d’URL vers `/clubs/[slug]/events/fallback-{n}` avec rendu événement fallback côté page event.
- Status: ✅ Resolved
---

---
**[2026-03-04] — Retrait du fallback temporaire et rétablissement du flux public data-driven**
- File(s) affected: `supabase/migrations/013_public_club_profiles_read.sql`, `supabase/migrations/014_public_read_grants.sql`, `supabase/migrations/015_public_read_policies_refresh.sql`, `src/app/(public)/clubs/[slug]/page.tsx`, `src/app/(public)/clubs/[slug]/events/[eventId]/page.tsx`
- Error: Le flux public `club → event` dépendait d’un fallback applicatif et retournait encore des 404 sur certaines pages event réelles.
- Root cause: Politiques/grants publics incomplets côté DB (`club_profiles`/`event_tables`) + requête événement avec colonne inexistante (`min_spend`) + normalisation trop stricte de la relation `table:tables`.
- Fix applied: Ajout/refresh des policies RLS publiques + grants lecture, suppression du fallback app (liens `fallback-*` et `serverAdmin`), correction de la requête events (colonnes valides), et normalisation robuste `table` (objet ou tableau).
- Status: ✅ Resolved
---

---
**[2026-03-04] — Entrées transitoires obsolètes dans le changelog**
- File(s) affected: `CHANGELOG.md`
- Error: La section `Unreleased` contenait encore des entrées intermédiaires (fallback temporaire) devenues contradictoires avec l’état final livré.
- Root cause: Accumulation d’itérations successives pendant le debug/QA sans consolidation documentaire finale.
- Fix applied: Nettoyage ciblé de `Unreleased` pour retirer les traces temporaires obsolètes et conserver une narration compacte de l’état final.
- Status: ✅ Resolved
---

---
**[2026-03-04] — Structure `Unreleased` répétitive (sous-sections `Changed` dupliquées)**
- File(s) affected: `CHANGELOG.md`
- Error: La partie basse de `Unreleased` conservait des sous-sections `Changed` répétées qui nuisaient à la lisibilité.
- Root cause: Empilement de journaux de session hétérogènes dans la même release non coupée.
- Fix applied: Déduplication structurelle des headers répétitifs, conservation des entrées utiles et simplification de la narration de release.
- Status: ✅ Resolved
---

---
**[2026-03-04] — `Unreleased` surchargé (redondance volumétrique persistante)**
- File(s) affected: `CHANGELOG.md`
- Error: Malgré une première consolidation, `Unreleased` restait trop long avec des dizaines d’entrées atomiques peu scannables.
- Root cause: Historisation très granulaire des itérations UI/debug sans regroupement par domaines.
- Fix applied: Compression "max" de `Unreleased` en synthèses Added/Changed/Fixed orientées domaines, en conservant les correctifs finaux clés et les migrations de référence.
- Status: ✅ Resolved
---

---
**[2026-03-04] — `PROJECT_STATUS` trop volumineux dans la section `Fait`**
- File(s) affected: `docs/PROJECT_STATUS.md`
- Error: La section `Fait` était devenue peu exploitable en review (liste très longue et répétitive).
- Root cause: Accumulation de micro-entrées de session sans consolidation périodique.
- Fix applied: Regroupement en synthèse par domaines (documentation, UX, dashboards, public booking, paiements, données/sécurité, qualité technique) tout en conservant les jalons clés.
- Status: ✅ Resolved
---
