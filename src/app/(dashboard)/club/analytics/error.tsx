...
type ClubAnalyticsErrorProps = {
  error: Error;
  reset: () => void;
};

export default function ClubAnalyticsError({ error, reset }: ClubAnalyticsErrorProps): ReactElement {
  return (
    <section className="rounded-xl border border-[#C4567A]/35 bg-[#1A1D24] p-8">
      <p className="text-xs uppercase tracking-[0.18em] text-[#C4567A]">Erreur analytics</p>
      <h2 className="mt-2 text-lg font-semibold text-[#F7F6F3] md:text-xl">Impossible de charger les analytics</h2>
      <p className="mt-3 text-sm text-[#888888]">
        {error.message || "Une erreur inattendue est survenue pendant le chargement des analytics."}
      </p>
      <button
        type="button"
        onClick={reset}
